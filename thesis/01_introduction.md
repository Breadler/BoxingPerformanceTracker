...
_Status: UNCHANGED (2 minor edits: dropped "3D" before "pose estimation";
final sentence's metric names updated to punch volume/guard height/movement)._
## 1.1 Background
...
_Status: UNCHANGED (1 stat correction: mean = 3.69110 to 3.69/5)._
...
For example, Forbes highlights a swing analysis app for tennis that
evaluates single-movement performance (Chan-Danisi, 2024). However, these
systems lack time-based
...
_Status: CHANGED (minor wording: dropped "3D," "multiple training rounds"
became "a training session")._
...
_Status: CHANGED. Objectives rewritten to name the actual deliverables
(pose-landmark labelling pipeline, on-device-ported RF classifier,
evaluation across the three real metrics) instead of the old generic
investigate/develop/evaluate framing._
...
_Status: CHANGED. Most bullets held; the classifier-deployment and
visualization bullets changed because the plan changed (§4.4.2, §4.4.5)._
...
- On-device pose estimation using MediaPipe Pose Landmarker. _(Originally
  ...
  accurate, since MediaPipe Tasks is the current API surface for the same
...
- Application of a Random Forest classifier to detect punch events, ported
  to run natively on-device (§4.4.2). _The original scope described
  "TensorFlow Lite" conversion specifically; this changed (§4.4.2 explains
...
- Visualization of results through a skeleton-overlay video synced to
  ...
  specifically was dropped, §4.4.5/§6.5, while the timeline and graphs
  held.)_
...
_Status: CHANGED. Old thesis had 5 chapters; this thesis has 6
(Implementation split out as its own chapter)._
## 1.6 Thesis Outline
_Rewritten, since the old 5-chapter outline (old thesis §1.6) predates the
...
- **Chapter 2. Literature Review.** Boxing fundamentals and terminology,
  ...
  sports-analysis research, a comparative analysis of existing boxing and
  fitness apps, and pilot-study findings on user needs.
- **Chapter 3. Methodology.** The planned research design and pipeline,
  covering data collection, landmark extraction, labelling, feature
  engineering, model development, per-metric computation, system design,
  and the planned evaluation strategy.
- **Chapter 4. Implementation.** What was actually built: the Python
  ...
  an Android application. This includes the specific technologies,
  algorithms, and calculations used, and the reasoning behind key
  implementation decisions.
- **Chapter 5. Results and Evaluation.** The actual output produced at
  every pipeline stage, and the results of evaluating the system against
  the strategy set out in Chapter 3, including label quality, model
  accuracy, on-device performance, metric validity, robustness, and
